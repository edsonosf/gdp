...
const UserEditForm: React.FC<UserEditFormProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    cargo: user.cargo || '',
    funcao: user.role || '',
    otherFuncao: user.cargo && !positions.some(p => p.value === user.role) ? user.role : '',
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-4 bg-slate-50 rounded-3xl border border-slate-100 space-y-4">
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {workSchedules.map(o => (
                        <button key={o.id} type="button" onClick={() => handleToggle(selectedCarga, setSelectedCarga, o.value)} className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${selectedCarga.includes(o.value) ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-slate-200 text-slate-600'}`}>
                            {o.value}
                        </button>
                    ))}
                </div>
        ...
        {showAdminToggle && (
            <div className="flex items-center space-x-3 p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
                <input type="checkbox" id="isSystemAdmin" name="isSystemAdmin" checked={formData.isSystemAdmin} onChange={handleInputChange} className="text-xs w-5 h-5 text-indigo-600 rounded font-medium" />
